...
JNIEnv* getEnv() {
    JNIEnv *env;
    int status = (*pj_jni_jvm)->GetEnv(pj_jni_jvm, (void **)&env, JNI_VERSION_1_4);
    //int status = gJvm->GetEnv((void**)&env, JNI_VERSION_1_6);
    if(status < 0) {    
        status = (*pj_jni_jvm)->AttachCurrentThread(pj_jni_jvm, &env, NULL);
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    if (!jni_env)
     ...
     uuid_class = (jclass)(*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "pl/orangelabs/wificalling/util/UUID"));
     classClass = (jclass)(*jni_env)->NewGlobalRef(jni_env, (*jni_env)->GetObjectClass(jni_env, uuid_class));
     
    classLoaderClass = (jclass)(*jni_env)->NewGlobalRef(jni_env, (*jni_env)->FindClass(jni_env, "java/lang/ClassLoader"));
    getClassLoaderMethod = (*jni_env)-> GetMethodID(jni_env, classClass, "getClassLoader", "()Ljava/lang/ClassLoader;");
    gClassLoader =  (jclass)(*jni_env)->NewGlobalRef(jni_env,(*jni_env)->CallObjectMethod(jni_env, uuid_class, getClassLoaderMethod));
    gFindClassMethod = (*jni_env)-> GetMethodID(jni_env, classLoaderClass, "findClass", "(Ljava/lang/String;)Ljava/lang/Class;");
...
jclass findClass(const char* name) {
    ...
    return (jclass)((*getEnv())->CallObjectMethod(getEnv(), gClassLoader, gFindClassMethod, (*getEnv())->NewStringUTF(getEnv(), name)));
