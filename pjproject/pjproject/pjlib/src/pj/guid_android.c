...
PJ_DEF(pj_str_t*) pj_generate_unique_string(pj_str_t *str)
{
    ...
    uuid_class = (jclass)(*jni_env)->NewGlobalRef(jni_env, findClass("pl/orangelabs/wificalling/util/UUID"));
     if (uuid_class == 0)
        goto on_error;
    to_string_method = (*jni_env)->GetStaticMethodID(jni_env, uuid_class,
    						"randomUUID",
    						"()Ljava/lang/String;");
    if (to_string_method == 0)
        goto on_error;
    uuid_string = (*jni_env)->CallStaticObjectMethod(jni_env, uuid_class, to_string_method);
    if (uuid_string == 0)
        goto on_error;
     /*   
