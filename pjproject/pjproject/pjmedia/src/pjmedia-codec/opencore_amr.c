...
enum
{
    ...
    IDX_AMR_NB_OLP,	/* Index for narrowband.    */
    IDX_AMR_WB,	/* Index for wideband.      */
    IDX_AMR_WB_OLP	/* Index for wideband.      */
};

static pjmedia_codec_amr_config def_config[4] =
{{ /* AMR-NB */
...
static const pj_uint16_t* amr_bitrates[4] =
    {pjmedia_codec_amrnb_bitrates,pjmedia_codec_amrnb_bitrates,  pjmedia_codec_amrwb_bitrates,pjmedia_codec_amrwb_bitrates};

static const unsigned amr_bitrates_size[4] =
{
...
#endif
#ifdef USE_AMRNB
    amr_codec_factory.init[IDX_AMR_NB_OLP] = ((options & PJMEDIA_AMR_NO_NB) == 0);
#else
    amr_codec_factory.init[IDX_AMR_NB_OLP] = PJ_FALSE;
#endif
#ifdef USE_AMRWB
...
#endif
#ifdef USE_AMRWB
    amr_codec_factory.init[IDX_AMR_WB_OLP] = ((options & PJMEDIA_AMR_NO_WB) == 0);
#else
    amr_codec_factory.init[IDX_AMR_WB_OLP] = PJ_FALSE;
#endif
...
{
    if (amr_codec_factory.init[IDX_AMR_NB] && amr_codec_factory.init[IDX_AMR_NB_OLP] &&
        amr_codec_factory.init[IDX_AMR_WB] && amr_codec_factory.init[IDX_AMR_WB_OLP])
    {
...
{
    return amr_set_config(IDX_AMR_NB_OLP, (const pjmedia_codec_amr_config *)config);
}

PJ_DEF(pj_status_t) pjmedia_codec_opencore_amrwb_set_config(
                                    const pjmedia_codec_amrwb_config *config)
{
    return amr_set_config(IDX_AMR_WB_OLP, (const pjmedia_codec_amr_config *)config);
}
				   ...
				   const pjmedia_codec_info *info )
{
    ...
    /* Check payload type. */
    if (info->pt != PJMEDIA_RTP_PT_AMR && info->pt != PJMEDIA_RTP_PT_AMR_OLP && info->pt != PJMEDIA_RTP_PT_AMRWB && info->pt != PJMEDIA_RTP_PT_AMRWB_OLP)
	return PJMEDIA_CODEC_EUNSUP;
    ...
    /* Check clock-rate */
    if ((info->clock_rate == 8000 && amr_codec_factory.init[IDX_AMR_NB]) || (info->clock_rate == 8000 && amr_codec_factory.init[IDX_AMR_NB_OLP]) ||
        (info->clock_rate == 16000 && amr_codec_factory.init[IDX_AMR_WB]) ||  (info->clock_rate == 16000 && amr_codec_factory.init[IDX_AMR_WB_OLP]))
    {
        return PJ_SUCCESS;
    }
				     ...
				     pjmedia_codec_param *attr )
{
    ...
    if(id->pt == PJMEDIA_RTP_PT_AMR)
    {
        idx = IDX_AMR_NB;
    }
    else if(id->pt == PJMEDIA_RTP_PT_AMR_OLP)
    {
        idx = IDX_AMR_NB_OLP;
    }
    else if(id->pt == PJMEDIA_RTP_PT_AMRWB)
    {
        idx = IDX_AMR_WB;
    }
    else
...
/*
	...
	attr->setting.dec_fmtp.param[1].name = pj_str("mode-change-capability");
	attr->setting.dec_fmtp.param[1].val = pj_str("2");
    }
else
	{
	attr->setting.dec_fmtp.cnt = 1;
	attr->setting.dec_fmtp.param[0].name = pj_str("mode-change-capability");
	attr->setting.dec_fmtp.param[0].val = pj_str("2");*/
}
				    ...
				    pjmedia_codec_info codecs[])
{
    ...
    *count = 0;
      ...
      if (amr_codec_factory.init[IDX_AMR_NB_OLP]) {
        pj_bzero(&codecs[*count], sizeof(pjmedia_codec_info));
        codecs[*count].encoding_name = pj_str("AMR");
        codecs[*count].pt = PJMEDIA_RTP_PT_AMR_OLP;
        codecs[*count].type = PJMEDIA_TYPE_AUDIO;
        codecs[*count].clock_rate = 8000;
        codecs[*count].channel_cnt = 1;
        (*count)++;
   ...
   if (amr_codec_factory.init[IDX_AMR_WB]) {
    ...
    if (amr_codec_factory.init[IDX_AMR_WB_OLP]) {
        pj_bzero(&codecs[*count], sizeof(pjmedia_codec_info));
        codecs[*count].encoding_name = pj_str("AMR-WB");
        codecs[*count].pt = PJMEDIA_RTP_PT_AMRWB_OLP;
        codecs[*count].type = PJMEDIA_TYPE_AUDIO;
        codecs[*count].clock_rate = 16000;
        codecs[*count].channel_cnt = 1;
        (*count)++;
				   ...
				   pjmedia_codec_param *attr )
{
    ...
    if(attr->info.pt == PJMEDIA_RTP_PT_AMR)
    {
        idx = IDX_AMR_NB;
    }
    else if(attr->info.pt == PJMEDIA_RTP_PT_AMR_OLP)
    {
        idx = IDX_AMR_NB_OLP;
    }
    else if(attr->info.pt == PJMEDIA_RTP_PT_AMRWB)
    {
        idx = IDX_AMR_WB;
    }
    else
    ...
    if (idx == IDX_AMR_NB || idx==IDX_AMR_NB_OLP) {
#ifdef USE_AMRNB
    ...
    setting->amr_nb = ((idx == IDX_AMR_NB||idx==IDX_AMR_NB_OLP)? 1: 0);
    setting->reorder = 0;
