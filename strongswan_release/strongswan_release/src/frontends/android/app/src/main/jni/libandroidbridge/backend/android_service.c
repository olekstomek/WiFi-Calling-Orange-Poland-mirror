							 ...
							 ike_sa_t *ike_sa, child_sa_t *child_sa)
{
	...
	{
			DBG1(DBG_DMN, "add_routes(builder, child_sa)");
		return FALSE;
	}
							...
							peer_cfg_t *peer_cfg, bool byod)
{
	...
	auth_cfg_t *auth;
	char *username, *password, *local_id, *aaa_identity;;
	...
	aaa_identity = this->settings->get_str(this->settings, "connection.aaa_identity", NULL);
    identification_t* aaa_identification = identification_create_from_string(aaa_identity);
    auth->add(auth, AUTH_RULE_AAA_IDENTITY, aaa_identification);

	this->creds->add_username_password(this->creds, username, password);
	DBG1(DBG_CFG, "xx olp_add_shared_from_cert");
	this->creds->olp_add_shared_from_cert(this->creds, local_id, password);
		DBG1(DBG_CFG, "xx add_auth_cfg");
...
static job_requeue_t initiate(private_android_service_t *this)
{
	...
	ike_cfg->add_proposal(ike_cfg, proposal_create_from_string(PROTO_IKE, "aes256-sha256-prfsha256-modp2048"));
	...
	//peer_cfg->add_virtual_ip(peer_cfg, host_create_any(AF_INET6));
	    DBG1(DBG_CFG, "xx connection.,type");
	type = this->settings->get_str(this->settings, "connection.type", NULL);
	/* local auth config */
		...
		streq("ikev2-eap-tls", type))
	{
	    ...
	    DBG1(DBG_CFG, "xx remote auth config");
	...
	child_cfg->add_proposal(child_cfg, proposal_create_from_string(PROTO_ESP,"aes256-sha256"));
