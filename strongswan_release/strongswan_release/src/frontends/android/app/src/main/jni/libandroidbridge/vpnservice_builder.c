...
static int establish_internal(private_vpnservice_builder_t *this, char *method)
{
	...
	if (!method_id)
	{
	    		DBG1(DBG_LIB, "!method_id");
		goto failed;
	}
	//OLP/ added server ip param to establish call
    char *serverIp = lib->settings->get_str(lib->settings, "olp.pcscf.serverip", "null");
    int ipsLength = strlen(serverIp);
    jstring str = (*env)->NewStringUTF(env, serverIp);
    if (!str)
    {
    		DBG1(DBG_LIB, "!str");
    	goto failed;
    }
	fd = (*env)->CallIntMethod(env, this->builder, method_id, str);
	if (fd == -1)
	{
		    		DBG1(DBG_LIB, "!method_id %s",method_id);
		DBG1(DBG_LIB, "builder: fd == -1");
		goto failed;
	}
	androidjni_detach_thread();
	return fd;
